...
volatile uint16_t gamepad_info;      // Host gamepad info
...
void parse_gamepad_report(uint8_t const *report, uint16_t len , uint8_t instance)
{
	...
	if (previous.value != current.value)
	{
        ...
        if(value&32) {gamepad_info&=0xffdf;};   // TRIG B
        if(value&16) {gamepad_info&=0xffef;};   // TRIG A
        if(value&8) {gamepad_info&=0xfffb;};   // Left
        if(value&4) {gamepad_info&=0xfffd;};   // Down
        if(value&2) {gamepad_info&=0xfff7;};  // Right
        if(value&1) {gamepad_info&=0xfffe;};  // Up
